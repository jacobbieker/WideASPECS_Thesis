...
The first challenge is to find signals in the datacubes. These datacubes are very noisy with the noise being symmetric around zero. Because of the size of the datacubes there should be roughly 65 features at a S/N $>$ 5 that are just noise peaks. As the S/N cut goes up though, the number of features goes down dramatically, with a S/N $>$ 6 resulting in only one feature per ~0.5 billion realizations, and so should happen with very rarely, if at all, in these datacubes.
