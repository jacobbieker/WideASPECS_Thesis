...
An example output from MAGPHYS is shown in Fig. \ref{fig:MAGPHYS_Example}, showing the model, the likelihood values for various physical properties of the galaxy, and the data points used to compute the SED in red. The distribution of the stellar mass, and star formation rates of all the galaxies fitted with MAGPHYS are shown in Fig. \ref{fig:MAGPHYS_Properties}. Galaxies whose computed star formation rate was at or below [INSERT CUTOFF POINT] were removed from the rest of the analysis, as this indicated a very poor MAGPHYS fit. This cutoff was at a log(SFR) of less than -2. That left a total of $~$55000 galaxies in the sample. 

With other quality cuts, the number of galaxies drops to 20,000. 
...
\begin{figure}[tbp]
\centering \includegraphics[width=120mm]{Survey/MAGPHYS_SFR.png}
\caption{Distribution of star formation rate for all galaxies in the catalog.}
\label{fig:MAGPHYS_Properties}
\end{figure}

\begin{figure}[tbp]
\centering \includegraphics[width=120mm]{Survey/MAGPHYS_Mstar.png}
\caption{Distribution of mass for all galaxies in the catalog.}
\label{fig:MAGPHYS_Properties}
\end{figure}
...
noise, a 5-sigma feature appears every ~1.74 million realizations (one
in 370 at S/N=3). The map is HUGE! We have an area of ~52 arcmin2,
which roughly corresponds ~40,000 times the synthesized beam area. We
have ~1900 independent channels in the sum of the four cubes analyzed
so far, so the total number of independent resolution elements in the
entire survey is ~77 millions. So, for a purely gaussian noise, you
would already expect ~45 line candidates at S/N=5

1) for gaussian noise, the significativity of line candidates
increases very quickly with S/N above S/N~5. E.g., a S/N=6 noise
features appears every ~0.5 billion realizations, so for many
respects, S/N>6 is a relatively safe ground even for our enormous
dataset.

    2) at the level of significance we're dealing with, with
practically "signal--free" cubes, the noise is symmetrical around
zero, so we can have a good sampling of the noise properties from the
statistics of negative features. In other words, we don't need to
assume gaussianity, we can simply fit the *observed* distribution of
noise candidates, with the only (very reasonable) assumption that the
noise is symmetric around zero. This translates into a definition of
"fidelity" or "reliability" of each line candidate, which is basically
the probability that a line candidate is *not* due to noise.]

[FIND MY RESPONSE AND NUMBER FOR 6 DATACUBES -> 60 something total?]

The first challenge is to find signals in the datacubes. These datacubes are very noisy and have [TOTAL RESOLUTION ELEMENTS], with the noise being symmetric around zero. This means that there should be roughly [ACTUAL NUMBER] features at a S/N $>$ 5 that are just noise peaks. As the S/N cut goes up though, the number of features goes down dramatically, with a S/N $>$ 6 resulting in only one feature per ~0.5 billion realizations. [DOUBLE CHECK AND MAKE CLEANER]
